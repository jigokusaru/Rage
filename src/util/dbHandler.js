...
class DbHandler {
        ...
        try {
            ...
            for (let column in desiredColumns) {
                if (!currentColumns.includes(column)) {
                    await conn.query(`ALTER TABLE users ADD COLUMN ${column} ${desiredColumns[column]}`);
            ...
            for (let column of currentColumns) {
                if (!Object.keys(desiredColumns).includes(column) && column !== 'id' && column !== 'discord_id') {
    ...
    async addUser(discord_id) {
        let rows = await this.runQuery('SELECT * FROM users WHERE discord_id = ?', [discord_id]);
        if (rows.length > 0) {
            throw new Error();
        } else {
