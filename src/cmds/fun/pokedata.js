...
const fun_pokedex = {
  ...
  execute: async (bot, cmd, args, msg) => {
    ...
    try {
      ...
      const genderRatio =
        genderRate >= 0
          ? `Male: ${genderRate * 12.5}%\n Female: ${(8 - genderRate) * 12.5}%`
          : "Genderless";

      // Fetch evolution chain data
      const evoChainData = await P.getEvolutionChainById(
        speciesData.evolution_chain.url.split("/")[6]
      );
      ...
      if (error.message.includes("Request failed with status code 404")) {
        msg.channel.send(`${pokemonName.charAt(0).toUpperCase() + pokemonName.slice(1)} is not a valid pokemon. Do you even know what a Pokémon is?`);
      } else {
